...
export const usePublicVaultsByTags = (params: PaginationInput) => {
  const { errorHandler } = useErrorHandler();
  const { getPublicVaultsByTagsQuery } = useVaultsActions();
  const { vaults, setVaults } = useVaultsStore();
  const [loading, setLoading] = useState(true);
  const [hasMore, setHasMore] = useState(false);
  ...
  const loadVaults = async (initialLoad = false) => {
    ...
    setLoading(vaults.length === 0);
    try {
      const { data } = await getPublicVaultsByTagsQuery({
        ...params,
        take: params.take ?? 40,
        skip,
        searchTerm: params.searchTerm,
      ...
      const fetched = (data?.getPublicVaultsByTags ?? []) as GetPublicVaultsOutput[];
      setHasMore(fetched.length === (params.take ?? 40));
      setVaults((prev) => (initialLoad ? fetched : [...prev, ...fetched]));
    } catch (error) {
      errorHandler({ error });
