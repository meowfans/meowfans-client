...
export default async function SingleChannelPage({ params }: SingleChannelProps) {
  ...
  const channel = await getSingleChannel({ relatedEntityId: id, take: 50 });
  ...
  return (
    <div className="flex h-full w-full overflow-hidden">
      ...
      <div className="hidden xl:block w-72 lg:w-80 flex-none h-full overflow-hidden bg-background">
        <FanDetails channel={channel} />
      </div>
    </div>
  );
}
