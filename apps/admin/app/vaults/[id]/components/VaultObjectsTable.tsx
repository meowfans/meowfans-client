...
export function VaultObjectsTable({
  ...
  const isAllSelected =
    vaultObjects.length > 0 && vaultObjects.every((obj) => obj.status === DownloadStates.Fulfilled || selectedObjects.includes(obj.id));
  ...
  return (
    <div className="flex-1 min-h-0 relative border rounded-xl bg-card overflow-hidden shadow-sm">
      {loading && vaultObjects.length === 0 ? (
        ...
        <InfiniteScrollManager useWindowScroll dataLength={vaultObjects.length} hasMore={hasNext} onLoadMore={onLoadMore} loading={loading}>
            ...
            <table className="w-full caption-bottom text-sm">
              <TableHeader className="sticky top-0 bg-card z-10 shadow-sm border-b">
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="w-20 md:w-32 px-2 md:px-4 py-3 md:py-4 text-center font-semibold text-xs md:text-sm">
                    Type
                  </TableHead>
                  <TableHead className="w-24 md:w-40 px-2 md:px-4 py-3 md:py-4 text-center font-semibold text-xs md:text-sm">
                    Status
                  </TableHead>
                  <TableHead className="min-w-[150px] px-2 md:px-4 py-3 md:py-4 font-semibold text-xs md:text-sm">Object URL</TableHead>
                  <TableHead className="text-right px-2 md:px-4 py-3 md:py-4 font-semibold whitespace-nowrap hidden sm:table-cell text-xs md:text-sm">
                    Created At
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {vaultObjects.map((obj) => (
                  ...
                  >
                    ...
                    <TableCell
                      className="px-2 md:px-4 py-2 md:py-3 font-mono text-[10px] md:text-xs text-muted-foreground max-w-[120px] md:max-w-[250px] truncate"
                      title={obj.objectUrl}
                    >
                      {obj.objectUrl || <span className="text-muted-foreground/30">No URL</span>}
                    </TableCell>
