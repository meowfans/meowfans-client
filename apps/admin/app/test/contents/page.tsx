    ...
    <div className="p-8 space-y-8 container mx-auto">
      <div className="space-y-2 border-b border-primary/10 pb-6">
        <h1 className="text-4xl font-black italic uppercase tracking-tighter">
          Test <span className="text-primary italic">Contents</span>
        </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
        {imagesData.map((item, index) => {
          const isVideo = item.raw_url.toLowerCase().endsWith('.mp4') || item.raw_url.toLowerCase().endsWith('.m4v') || item.raw_url.toLowerCase().endsWith('.mov');
          
          return (
            <Card key={item.id || index} className="overflow-hidden border-primary/5 hover:border-primary/20 transition-all duration-500 bg-secondary/10 group relative aspect-[9/16] flex flex-col">
              ...
              <div className="p-4 bg-background/80 backdrop-blur-md border-t border-primary/5 space-y-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-[8px] font-mono text-muted-foreground/40 break-all truncate max-w-[100px]">
                    {item.id}
                  </span>
                </div>
